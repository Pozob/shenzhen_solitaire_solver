...
function cardEditClick(event) {
    ...
    selectorEl.css('top', event.clientY).css('left', event.clientX).show().focus();
    selectorEl.find("img[alt='" + this.title + "']").addClass('selected');
    if (this.innerHTML != '') suitNumberInputEl.val(this.innerHTML);
...
$(document).ready(function () {
    ...
    $('#selector .suits img').click(function () {
        ...
        $(this).addClass('selected');
        suitNumberInputEl.select().focus();
    });
    selectorEl.keydown(function (event) {
        switch (event.key) {
            ...
            case 'Escape':
                ...
                break;
            case '1':
                applyButtonTimeOut = setTimeout(() => applySuitBtnEl.click(), 500);
                break;
            case '2':
            case '3':
            case '4':
            ...
            case '9':
            case '0':
                setTimeout(() => applySuitBtnEl.click(), 0);
                clearTimeout(applyButtonTimeOut);
        }
    });
    applySuitBtnEl.click(function () {
        if (selectedSuit == null) alert('Please select a suit');
        else {
    ...
    let interval = null;
    document.querySelector('.js-start-autoscroller').addEventListener('change', e => {
        if (e.target.checked) {
            const waitTime = document.querySelector('.js-autoscroller-wait-time').value * 1000;
    ...
    function selectNextCard(currentCard) {
        let newSelectedCardEl = currentCard.next();
        if (newSelectedCardEl.length == 0) {
            const nextParent = currentCard.parent().next();
            if (nextParent.length == 0) return;
            newSelectedCardEl = nextParent.children().first();
        }
        cardEditClick.call(newSelectedCardEl[0], { clientX: newSelectedCardEl.offset().left, clientY: newSelectedCardEl.offset().top + 30 });
    }
});
...
function prepare_solution(solutionStates) {
    ...
    for (let i = 0; i < solutionStates.length; i++) {
        ...
        if ('from' in state.action) {
            ...
            if ('tray' in state.action.from) {
                if (state.action.from.count > 1) text += state.action.from.count + ' cards ';
                else text += 'a card ';
            ...
            text += ' to ';
            if ('tray' in state.action.to) text += '<b>tray ' + (state.action.to.tray + 1) + '</b>';
    ...
    solutionBoxEl.next_page = function () {
        if (this.currentPage < this.maxPage) this.open_step(++this.currentPage);
    };
    solutionBoxEl.prev_page = function () {
        if (this.currentPage > 0) this.open_step(--this.currentPage);
    };
    solutionBoxEl.open_step = function (num) {
        ...
        if ('from' in action) {
            if ('tray' in action.from) $('#trays #tray-' + action.from.tray + ' :nth-last-child(-n + ' + action.from.count + ')').addClass('selected-from');
            else if ('slot' in action.from) $('#slots #slot-' + action.from.slot + ' .card').addClass('selected-from');
            if ('tray' in action.to) $('#trays #tray-' + action.to.tray + ' :last-child, #trays #tray-' + action.to.tray).addClass('selected-target');
            else if ('slot' in action.to) $('#slots #slot-' + action.to.slot + ' .card').addClass('selected-target');
            ...
            $('#tray-' + action.pop + ' :last-child').addClass('selected-from');
        }
    };
